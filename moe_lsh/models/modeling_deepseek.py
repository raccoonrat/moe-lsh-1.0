...
class DeepseekModel(DeepseekPreTrainedModel):
    ...
    def forward(
        ...
        output_router_weights: Optional[bool] = None # <-- 新增参数
        ...
        for decoder_layer in self.layers:
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if output_router_weights and sent_emb is not None:
                    collected_router_weights.append(sent_emb.unsqueeze(1))
        ...
        if output_router_weights:
            if collected_router_weights: # 检查列表是否为空
                sent_embs = torch.cat(collected_router_weights, dim=1)
        ...
        if not return_dict:
            if output_router_weights:
                standard_outputs = tuple(v for v in [hidden_states, next_cache, all_hidden_states, all_self_attns] if v is not None)
                return standard_outputs + (sent_embs,)
            else:
        ...
        # 2. 总是准备 sent_embs (即使是 None)
        sent_embs = None
        if output_router_weights and collected_router_weights: # (假设 collected_router_weights 是你用来收集的列表)
            sent_embs = torch.cat(collected_router_weights, dim=1)

        # 3. 总是返回一个元组
        return output_obj, sent_embs
...
class DeepseekForCausalLM(DeepseekPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        outputs, sent_embs = self.model(
            ...
            output_router_weights=output_router_weights, # <-- 传递新参数
        )

        hidden_states = outputs.last_hidden_state # <--- 关键修正：从对象中获取 hidden_states
        if self.config.pretraining_tp > 1:
        ...
        # --- 关键修正: 修改最终的返回逻辑 ---
        if not return_dict:
        # 原始的 return_dict=False 逻辑可能也需要调整，但 generate 默认用 return_dict=True
        # 我们暂时只关心 return_dict=True 的情况
            pass
    ...
    def prepare_inputs_for_generation(
        ...
        if past_key_values is not None:
            if isinstance(past_key_values, Cache):
                ...
                max_cache_length = getattr(past_key_values, 'max_len', None)
            else:
